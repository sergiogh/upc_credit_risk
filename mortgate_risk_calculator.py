...
class CreditRisk():
    ...
    def __init__(self):
        ...
        #self.backend = QuantumInstance(device_backend)

    def get_classical_expectation_loss(self, uncertainty_model, K):
    ...
    def get_weighted_adder(self, K):
        return WeightedAdder(self.n_z + K, [0]*self.n_z + self.loss_given_default)

    def get_quantum_expected_loss(self, uncertainty_model, weighted_adder, loss_given_default):
        ...
        result = iae.run(quantum_instance=self.backend, shots=100)
    ...
    def get_quantum_var(self, losses, uncertainty_model, weighted_adder):
    ...
    def get_quantum_cvar(self, var, estimated_probability, uncertainty_model, weighted_adder):
        breakpoints = [0, var]
        slopes = [0, 1]
        offsets = [0, 0]
        f_min = 0
    ...
    def run(self, alpha, mortgages):
        ...
        expected_loss, exact_var, exact_cvar, p_l_less_than_var, losses     = self.get_classical_expectation_loss(uncertainty_model, K)
        weighted_adder                                                      = self.get_weighted_adder(K)
        confidence_expected_loss, expected_loss_estimation                  = self.get_quantum_expected_loss(uncertainty_model, weighted_adder, self.loss_given_default)    
        estimated_var, estimated_var_probability                            = self.get_quantum_var(losses, uncertainty_model, weighted_adder)
        estimated_cvar                                                      = self.get_quantum_cvar(estimated_var, estimated_var_probability, uncertainty_model, weighted_adder)
